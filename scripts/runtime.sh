#!/usr/bin/env bash
# Cross-platform runtime layer — sourced by all hooks and scripts.
# Provides OS detection, Python dispatcher, and portable utilities.
...
# Convert a POSIX /x/... path to a Windows drive path for the py launcher.
  ...
  case "${value}" in
    /[A-Za-z]/*)
      if command -v cygpath >/dev/null 2>&1; then
        ...
        printf '%s:%s\n' \
          "$(printf '%s' "${value}" | cut -c2 | tr '[:lower:]' '[:upper:]')" \
          "$(printf '%s' "${value}" | cut -c3-)"
      fi
      ;;
    *) printf '%s\n' "${value}" ;;
  esac
...
# Portable Python runner: handles exe, py-launcher, or unavailable.
run_python() {
  if [ "${PYTHON_MODE:-}" = "py-launcher" ]; then
    if [ "$#" -gt 0 ]; then
      case "$1" in
        ...
        *)
          local first
          first="$(_lstack_py_launcher_path_arg "$1")"
          shift
          py -3 "${first}" "$@"
          ;;
      esac
...
# Human-readable label for the active Python provider.
...
iso_now() {
  run_python -c "from datetime import datetime,timezone; print(datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'))" 2>/dev/null \
...
# Normalize a path to POSIX /drive/... form (used by freeze path matching).
normalize_hook_path() {
